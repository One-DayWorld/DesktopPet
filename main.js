...
const { buildRecentHistoryForPrompt } = require('./chat-history');
const { deleteChatTurn, approveAssistantReply, buildApprovedRepliesInject } = require('./chat-feedback');
const {
  buildProfileInject,
  buildRefineProfilePrompt,
  stripCodeFence
} = require('./memory-prompts');
...
function ensureClaudeHooksInstalled() {
  try {
    ...
    // 3) 通用安装函数: 给指定的 JSON 配置文件注入 / 更新 hook
    function installHooks(configPath) {
      if (!fs.existsSync(path.dirname(configPath))) {
        fs.mkdirSync(path.dirname(configPath), { recursive: true });
      }
      let settings = {};
      if (fs.existsSync(configPath)) {
        try { settings = JSON.parse(fs.readFileSync(configPath, 'utf8')); } catch (_) { settings = {}; }
      }
      settings.hooks = settings.hooks || {};

      // 删掉所有指向通知脚本的旧条目, 再加入 desired
      ...
      if (changed) {
        ...
        console.log('[HOOK] hooks installed/updated in', configPath);
      } else {
        console.log('[HOOK] hooks already up-to-date in', configPath);
      }
    }
...
function nextChatTurnId() {
  _chatTurnSeq += 1;
  return `turn-${Date.now().toString(36)}-${_chatTurnSeq.toString(36)}`;
}
...
ipcMain.handle('toggle-panel', () => {
  ...
  // 精致面板布局: 不再全屏遮挡，缩减为 380x580 卡片并贴近宠物显示，文字回话更聚焦。
  ...
  // 智能靠齐：将面板定位在宠物窗口左侧，底部与宠物对齐
  let px = petBounds.x - PW - 12;
  let py = petBounds.y + petBounds.height - PH;
  
  // 边界安全防护：确保面板不超出当前屏幕工作区
  if (px < sx) {
    // 如果左侧放不下，尝试放在宠物右侧
    if (petBounds.x + petBounds.width + PW + 12 <= sx + sw) {
      px = petBounds.x + petBounds.width + 12;
    } else {
      px = sx + 12; // 实在没空间则左靠齐
    }
  }
...
- 两人对话内没有明确第三方时, 不要新开第三人称角色; 用户提到”他/她/桌宠/你/他或她的东西/你的东西”时, 通常指向你这个助手/角色这一侧, 不是用户本人; 回复时要回到”我/你”两人视角, 不要继续写”他/她”。
...
- 只回答用户实际问的问题，不要主动补充无关信息`;
  ...
  const recentHistory = buildRecentHistoryForPrompt(history, 60000);
...
// ── 用户画像记忆: 注入 + 提炼 ────────────────────────────────────────────────
// 只用于 Chat 主路径, 不碰告警/任务播报 (那些走 voice-lines).
...
async function refineProfile(provider, apiKey, petName, oldMemory, newTurns, metasoKey = '') {
  ...
  const prompt = buildRefineProfilePrompt(oldMemory, newTurns);

  try {
    const raw = await callAI(provider, apiKey, petName, [], prompt.user, metasoKey, {
      systemOverride: prompt.system, noTools: true, temperature: 0.3
    });
    const txt = stripCodeFence(raw);
    return txt || null;
  } catch (e) {
...
[用户原话] ${message}`;
    ...
    try {
      const profileInject = buildProfileInject(cfg.memory, !!callOpts.persona);
      const approvedInject = buildApprovedRepliesInject(state.approvedReplies);
      callOpts.profileInject = [profileInject, approvedInject].filter(Boolean).join('\n\n');
    } catch (_) {}
    ...
    state.chatHistory.push({ id: `${turnId}-user`, turnId, role: 'user', content: message });
    state.chatHistory.push({ id: `${turnId}-assistant`, turnId, role: 'assistant', content: reply });
    ...
    return { reply, turnId, xp: state.pet.xp, level: state.pet.level };
  } catch (e) {
    return { error: e.message || '对话失败，请检查 API Key 或网络' };
  }
});

ipcMain.handle('delete-chat-turn', (_, turnId) => {
  const result = deleteChatTurn(state, turnId);
  if (result.deleted > 0) {
    state.approvedReplies = (state.approvedReplies || []).filter(s => s && s.turnId !== turnId);
    store.save(state);
  }
  return { ok: result.deleted > 0, deleted: result.deleted };
});

ipcMain.handle('approve-ai-reply', (_, turnId) => {
  const result = approveAssistantReply(state, turnId);
